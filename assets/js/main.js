...
function buildToc(){
  ...
  const threshold = 90; // debe superar el alto del nav fijo

  function updateActive(){
    let current = headingsArr[0];
...
document.querySelectorAll("[data-md-src]").forEach(async (el) => {
  ...
  try {
    ...
    // Las rutas de imagen del .md son relativas a la carpeta donde vive
    // el .md (ej. writeups/md/), no a la página HTML que lo incluye
    // (ej. writeups/boardlight.html) — hay que anteponer esa carpeta.
    const baseDir = src.includes("/") ? src.slice(0, src.lastIndexOf("/") + 1) : "";
    let html = typeof marked !== "undefined" ? marked.parse(raw) : `<pre>${raw.replace(/</g, "&lt;")}</pre>`;
    if (baseDir){
      html = html.replace(
